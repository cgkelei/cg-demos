...
vec3 ReconstructWorldPosition(float zBuffer, uvec2 pixelCoord)
{
	vec2 clipPos = (vec2(pixelCoord) + 0.5) * ViewportDim.zw; // InvViewDim
...
vec3 ReconstructViewPosition(float zBuffer, uvec2 pixelCoord)
{
	vec2 clipPos = (vec2(pixelCoord) + 0.5) * ViewportDim.zw; // InvViewDim
    clipPos = clipPos * 2.0 - 1.0;
...
void EvalulateAndAccumilateLight(Light light, vec3 litPos, vec3 N, vec3 V, float shininess,
                                 out vec3 diffuseLight, out vec3 specularLight)
{
...
void main()
{
	...
	float zBuffer = texelFetch(DepthBuffer, sampleIndex, 0).r;
	vec3 worldPosition = ReconstructWorldPosition(zBuffer, sampleIndex);
	...
	vec4 frustumPlanes[6];
	{
		...
		frutumCornerPosVS[0] = ReconstructViewPosition(1.0, uvec2(gl_WorkGroupID.x * gl_NumWorkGroups.x, gl_WorkGroupID.y * gl_NumWorkGroups.y));
		frutumCornerPosVS[1] = ReconstructViewPosition(1.0, uvec2((gl_WorkGroupID.x+1) * gl_NumWorkGroups.x, gl_WorkGroupID.y * gl_NumWorkGroups.y));
		frutumCornerPosVS[2] = ReconstructViewPosition(1.0, uvec2((gl_WorkGroupID.x+1) * gl_NumWorkGroups.x, (gl_WorkGroupID.y+1) * gl_NumWorkGroups.y));
		frutumCornerPosVS[3] = ReconstructViewPosition(1.0, uvec2(gl_WorkGroupID.x * gl_NumWorkGroups.x, (gl_WorkGroupID.y+1) * gl_NumWorkGroups.y));
	...
	if (all(lessThan(gl_GlobalInvocationID.xy, ViewportDim.xy)))
	{
		...
		vec4 normalShininess = texelFetch(GBuffer0, sampleIndex, 0);
