...
void DotSceneLoader::processScene(rapidxml::xml_node<>* XMLRoot)
{
	...
	pElement = XMLRoot->first_node("enemy");
	if(pElement)
		processEnemy(pElement);
...
//////////////////////////////////////////////////////////////////////////
// ruan add
void DotSceneLoader::processEnemy( rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent /*= 0*/ )
{
	String emenyName;
	String entityName;
	String meshName;
	...
	if(pElement)
	{
		quaternion = parseQuaternion(pElement);
	}
	...
	if(pElement)
	{
		...
		Ogre::String id = getAttrib(pElement, "id");
		bool castShadows = getAttribBool(pElement, "castShadows", true);
		meshName = getAttrib(pElement, "meshFile");
		meshMaterialName = getAttrib(pElement, "materialFile");
		bool isStatic = getAttribBool(pElement, "static", false);;
		

	}

	Enemy* ememy = new Enemy(ShipGame::getSingleton().GetSceneManager(), meshName, position, scale, quaternion, meshMaterialName);
	ShipGame::getSingleton().GetGameState()->AddEnemy(ememy);
