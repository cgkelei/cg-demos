...
OpenGLTexture1D::OpenGLTexture1D( PixelFormat format, uint32_t arraySize, uint32_t numMipMaps, uint32_t width, uint32_t sampleCount, uint32_t sampleQuality, uint32_t accessHint, uint32_t flags, ElementInitData* initData )
	: OpenGLTexture(TT_Texture1D, format, arraySize, numMipMaps, sampleCount, sampleQuality, accessHint, flags)
{
	TextureTarget = (mTextureArraySize > 1) ? GL_TEXTURE_1D_ARRAY : GL_TEXTURE_1D;
	...
	for (uint32_t arrIndex = 0; arrIndex < mTextureArraySize; ++ arrIndex)
	{
		for (uint32_t level = 0; level < mMipLevels; ++ level)
		{
			...
			else
			{
				...
				{
					if (0 == arrIndex)
						glTexImage2D(TextureTarget, level, internalFormat, levelWidth, mTextureArraySize, 0, externFormat, formatType, NULL);

					glTexSubImage2D(TextureTarget, level, 0, arrIndex, levelWidth, 1,
						externFormat, formatType, (NULL == initData) ? NULL : initData[arrIndex * mMipLevels + level].pData);
				}
				else
				{
					glTexImage1D(TextureTarget, level, internalFormat, levelWidth, 0, externFormat, formatType,
