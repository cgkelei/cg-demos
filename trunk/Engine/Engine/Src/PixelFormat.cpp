...
namespace Engine
{
	...
	*/
	...
	Engine::uint32 PixelFormatUtils::GetNumElemBytes( PixelFormat format )
	{
		 return GetDescriptionFor(format).elemBytes;
	}


	Engine::uint32 PixelFormatUtils::GetNumElemBits( PixelFormat format )
	{
		return GetDescriptionFor(format).elemBytes * 8;
	}

	Engine::uint32 PixelFormatUtils::GetMemorySize( uint32 width, uint32 height, uint32 depth, PixelFormat format )
	{
	...
	Engine::uint32 PixelFormatUtils::GetFlags( PixelFormat format )
	{	
	...
	Engine::String PixelFormatUtils::GetFormatName( PixelFormat srcformat )
	{
		return GetDescriptionFor(srcformat).name;
	}

	Engine::PixelComponentType PixelFormatUtils::GetComponentType( PixelFormat fmt )
	{
		const PixelFormatDescription &des =  GetDescriptionFor(fmt);
		return des.componentType;
	}

	Engine::uint32 PixelFormatUtils::GetComponentCount( PixelFormat fmt )
	{
		const PixelFormatDescription &des = GetDescriptionFor(fmt);
	...
	void PixelFormatUtils::GetNumDepthStencilBits( DepthFormat format, uint32& depth, uint32& stencil )
	{
		switch(format)
		{
		case DF_Depth15Stencil1:
			depth = 15;
			stencil = 1;
			break;
		case DF_Depth16:
